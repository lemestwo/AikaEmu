...
{
    public class ReinforceAData : BaseData<ReinforceEquipModel>
    {
        private readonly Logger _log = LogManager.GetCurrentClassLogger();

        public ReinforceAData(MySqlConnection connection)
        {
            using (var command = connection.CreateCommand())
            {
                command.CommandText = "SELECT * FROM `data_reinforce_a`";
                command.Prepare();
