...
namespace AikaEmu.GameServer.Models.Char.Inventory
{
    public class CharInventory
    {
        ...
        public bool Swap(SlotType slotType, ushort slotFrom, ushort slotTo)
        {
            ...
            using (var command = sql.CreateCommand())
            {
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        if ((item.SlotType != SlotType.Equipments || item.Slot <= 16) && (item.SlotType != SlotType.Inventory || item.Slot <= 85))
                        {
