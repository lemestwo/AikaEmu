/*
 * BIG THANKS TO MR.NIKES FROM AAEMU FOR PORTING TO C# AND
 * IMPLEMENTING NEW FUNCTIONS TO THIS CLASS
 */
...
namespace AikaEmu.GameServer.Utils
{
    public class IdFactory
    {
        ...
        private IEnumerable<uint> ExtractUsedObjectIdTable()
        {
            ...
            using (var connection = GameServer.Instance.DatabaseManager.GetConnection())
            {
                using (var command = connection.CreateCommand())
                {
                    var query = "SELECT " + (_distinct ? "DISTINCT " : "") + _objTables[0, 1] + ", 0 AS i FROM " + _objTables[0, 0];
                    for (var i = 1; i < _objTables.Length / 2; i++)
                        query += " UNION SELECT " + (_distinct ? "DISTINCT " : "") + _objTables[i, 1] + ", " + i + " FROM " + _objTables[i, 0];

                    command.CommandText = "SELECT COUNT(*), COUNT(DISTINCT " + _objTables[0, 1] + ") FROM ( " + query + " ) AS all_ids";
                    command.Prepare();
                    int count;
